...
do
    ...
    do
        ...
        echo "PARAMS = " $N $size
        GC=1000
        TIME_MEM_FILE=time_with_neutral_all_neutral.N$N"."size$size".out"
        if [ $KILL == 0 ]
        then
            DETAILED_TIME_FILE=detailed_time_argi_with_neutral_all_neutral.N$N"."size$size".out.gz"
            /usr/bin/time -f "%e %M" -o $TIME_MEM_FILE parallel --timeout $SECONDS_TO_KILL PYTHONPATH=../.. python ../../benchmarking.py --popsize $N --neutral_mutations --theta $size --rho $size --nsam 100 --pdel 0.0 --gc $GC --outfile1 $DETAILED_TIME_FILE --seed ::: 42 
